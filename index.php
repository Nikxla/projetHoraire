...
<div class="container align-content-center align-items-center text-center mt-5">
    ...
    <form action="index.php" method="post">
        <div class="col-md-8 d-inline-block">
            <select class="mdb-select md-form" id="mySelect" name="student">
                <option value="" disabled selected>Choix de l'étudiant</option>
            </select>
        </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                switch (data.ReturnCode) {
                    ...
                    case 1 : // un problème
                        break;
                    case 3 : // pas possible d'encoder json sur le serveur
                        alert(data.Message);
                        break;
                }
            }
    ...
    function displayStudent(data) {
        ...
        for (i = 0; i < data.Data.length; i++) {
            var o = new Option(data.Data[i].nom + " " + data.Data[i].prenom, data.Data[i].id + '-' + data.Data[i].IDGroupe + '-' + data.Data[i].nom + "-" + data.Data[i].prenom);
            $(o).html(data.Data[i].nom + " " + data.Data[i].prenom);
            $("#mySelect").append(o);
        }
    }
